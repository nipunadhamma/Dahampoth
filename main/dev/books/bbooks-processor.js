...
const MDI = (name, cls) =>
  `<i class="material-icons${cls ? " " + cls : ""}">${name}</i>`;
...
const mammothOpts = {
  styleMap: [
    ...
    "p[style-name='Title'] => h1.title", // මෙය එකතු කරන්න
...
(async () => {
  for (const book of bookList) {
    ...
    if (book.gen == "docx") {
      // reprocess docx or read from file (DANGER: do not do this for existing files)
      ...
      fs.writeFileSync(
        `${__dirname}/input/${book.folder}.html`,
        pretty(mRes.value),
        { encoding: "utf-8" },
      );
    } else {
      const bookDom = new JSDOM(
        fs.readFileSync(`${__dirname}/input/${book.folder}.html`, {
          encoding: "utf8",
        }),
      );
...
async function generateFiles(book, bookDoc) {
  const bookHeaders = $("h1,h2,h3,h4", bookDoc).get(),
    toc = JC("ul", "TOC-container"),
    filePath = `${__dirname}/files/${book.files[0]}`;
  ...
  const placeholders = {
    ...
    author: book.author,
    folder: book.folder,
    toc: pretty(JC("div").append(toc).html()),
  ...
  const fileBase = `${filePath}/${book.name}`,
    author = book.group != 2 && book.author ? ` - ${book.author}` : "",
...
function generateWebpages(book, bookDoc) {
  ...
  assert(
    nodesAdded == $("h1,h2,h3,h4", bookDoc).length,
    `nodes added (${nodesAdded}) not equals num headers (${$("h1,h2,h3,h4", bookDoc).length})`,
  );
...
function getBottomLinks(node) {
  ...
  return JC("nav", "bottom").append(
    prev.length
      ? JC("a", "button prev")
          .append(MDI("arrow_back"), prev.text())
          .attr("href", prev.attr("file"))
      : "",
    next.length
      ? JC("a", "button next")
          .html(`${next.text()} ${MDI("arrow_forward")}`)
...
function writeIndexFile(book, nodeList, fileName) {
  const patunaDiv = $("<div/>").append(
    nodeList.map((node) => createIndexDiv(node, { book, nodeList })),
  );
  const nameAuthor = `${book.name} - ${book.author}`;
  genericWriteFile(
    fileName,
    patunaDiv,
    fs.readFileSync(`${__dirname}/pre-book-index.html`, { encoding: "utf8" }),
    {
      title: nameAuthor,
      desc: nameAuthor,
      folder: book.folder,
